...
**建议**: 可以考虑检查 session 是否存在，不存在时返回 401 或直接返回成功

---

### 问题11: SECRET_KEY 硬编码在代码中
...
2. **问题 #2** (管理员统计权限) - 核心功能阻塞，用户体验严重受损
3. **问题 #5** (权限赋予) - 核心功能不可用，需先执行迁移脚本
4. **问题 #3** (删除成员功能缺失) - 基本 CRUD 功能不完整
5. **问题 #4** (编辑成员功能缺失) - 基本 CRUD 功能不完整
6. **问题 #6** (自引用检测) - 数据完整性
7. **问题 #7** (出生年份验证) - 数据合理性
8. **问题 #8** (测试代码修复) - 测试正确性
9. **问题 #9-12** - 安全性改进（实验项目可后续处理）
...
| 输入验证缺失 | #1, #6, #7 | 37.5% |
| 权限逻辑不完整 | #2 | 12.5% |
| 数据库Schema与代码不一致 | #5 | 12.5% |
| 前端功能缺失 | #3, #4 | 25% |
| 测试代码缺陷 | #8 | 12.5% |

### 代码层面根因

1. **Service层验证不足**: 多个业务函数缺少必要的输入验证（#1, #6, #7）
2. **权限检查不一致**: 统计功能未使用统一的 `check_access` 函数（#2）
3. **数据库迁移缺失**: `schema.sql` 与实际需要的 ENUM 不一致（#5）
4. **前端功能不完整**: 只实现了部分 CRUD 操作（#3, #4）
5. **测试代码缺陷**: 测试用例缺少前置条件（建立关系）导致断言失败（#8）

---

## 📝 验证说明

本报告在 2026年5月12日 进行了二次验证，通过完整阅读源代码确认所有12个问题均真实存在。
...
| #3 | ✅ 确认存在 | 阅读 `genealogy_detail.html`，确认操作列只有"详情"按钮 |
| #4 | ✅ 确认存在 | 同上 |
| #5 | ✅ 确认存在 | 阅读 `schema.sql:74`，确认 ENUM 不包含 'admin' 和 'owner' |
| #6 | ✅ 确认存在 | 阅读 `relation_service.py:41-58`，确认缺少自引用检查 |
| #7 | ✅ 确认存在 | 阅读 `member_service.py:57`，确认上界为 2100 |
| #8 | ✅ 确认存在 | 阅读 `test_relation.py:181-188`，确认测试缺少建立关系步骤 |
| #9 | ✅ 确认存在 | 阅读 `auth_service.py:9-11`，确认使用 `hashlib.sha256` |
| #10 | ✅ 确认存在 | 阅读 `auth_routes.py:59-63`，确认未检查 session |
| #11 | ✅ 确认存在 | 阅读 `config.py:17`，确认硬编码 |
| #12 | ✅ 确认存在 | 阅读 `config.py:11`，确认硬编码 |
